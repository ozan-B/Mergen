...
##Bu trap komutu, kullanıcı Ctrl+C kombinasyonunu girdiğinde temiz bir çıkış yapmayı amaçlar.

#trap        -->     bash te bir sinyali yakalamak için komuttur.
#printf "\n" -->     Yeni bir satır karakteri yazdırarak terminal ekranını düzeltir. Bu, kullanıcının daha temiz bir terminal ekranı görmesini sağlar.
#partial     -->     Önceden tanımlı partial fonksiyonunu çağırır. 
#exit 1      -->     Script'in çalışmasını sonlandırır ve çıkış kodunu "1" olarak ayarlar. Bu, bir hata durumunu temsil eder.
#2           -->     Bu, hangi sinyalin (SIGINT - Ctrl+C'ye karşılık gelen sinyal) yakalanacağını belirten bir sayıdır. Yani, bu durumda Ctrl+C kombinasyonu (SIGINT) yakalanır ve belirtilen işlemler gerçekleştirilir.


banner() {

figlet BoeWolf | lolcat 

#figlet Error  -->   Bu, "Error" metnini büyük ASCII sanatı ile yazdıran figlet komutudur.
#lolcat        -->   Bu, renkli metinleri renkli bir şekilde ekrana yazdıran bir araçtır. 
...
#Bu kontrol yapısı, belirtilen dosya varsa bir mesajı ekrana yazdırır. Mesaj, yeşil renkte "[*] Saved: dosya_adı.txt" şeklinde görüntülenir. 

#if [[ -e $username.txt ]]; then  -->  BU bir if kontrol yapısıdır. Eğer koşul true ise içindeki kodu çalıştırır.
#-e $username.txt                 --> BU belirtilen dosyanın var olup olmadığını kontrol eden komuttur.
...
read -p $'\e[1;92m[\e[0m\e[1;77m?\e[0m\e[1;92m] Input Username:\e[0m ' username   #kullanıcı adı istenir

if [[ -e $username.txt ]]; then  ##Bu satır, kullanıcı adının ismiyle aynı adı taşıyan bir dosyanın mevcut olup olmadığını kontrol eder.
printf "\e[1;92m[\e[0m\e[1;77m*\e[0m\e[1;92m] Removing previous file:\e[0m\e[1;77m %s.txt" $username   #dosyayı sileceğine dair bir bilgilendirme verir
rm -rf $username.txt  # Eğer dosya mevcut ise, önceki dosyayı silme işlemini bildiren bir çıktı verir.
fi
printf "\n"
printf "\e[1;92m[\e[0m\e[1;77m*\e[0m\e[1;92m] Checking username\e[0m\e[1;77m %s\e[0m\e[1;92m on: \e[0m\n" $username  #Kullanıcının adının kullanıldığı bir dosya oluşturma işlemini bildiren bir çıktı verir.
...
# Instagram kullanıcı adını kontrol etmek için bir URL oluştur
...
# Bu komut, belirli bir Instagram kullanıcı adını içeren bir URL'ye HTTP isteği gönderir ve sayfanın HTML içeriğini çeker. 
#-s seçeneği, çıktının sessiz modda (yani hata mesajları olmadan) alınmasını sağlar.
# -H "Accept-Language: en" seçeneği, dil tercihini belirtir.

# grep -o 'The link you followed may be broken'; echo $? -->  Bu, en son çalıştırılan komutun çıkış durumunu (exit status) yazdırır. Eğer grep belirtilen metni bulursa, çıkış durumu 0 olacaktır; bulamazsa çıkış durumu 1 olacaktır.


# Instagram varlığını kontrol etme sonucunu ekrana yazdırma
printf "\e[1;77m[\e[0m\e[1;92m+\e[0m\e[1;77m] Instagram: \e[0m"

# Kontrol sonucuna göre işlem yapma
if [[ $check_insta == *'1'* ]]; then # değişkeni, kontrolün sonucunu içerir (0 veya 1).
    printf "\e[1;92m Found!\e[0m https://www.instagram.com/%s\n" $username  #Eğer $check_insta içinde '1' varsa (yani belirli metin bulunursa), Instagram kullanıcı adının bulunduğunu belirten bir çıktı verir ve kullanıcı adını bir dosyaya yazar.
...
check1=$(curl -s -i "https://www.reddit.com/user/$username" -H "Accept-Language: en" -L --user-agent '"Mozilla/5.0 (X11; U; Linux i686; en-US; rv:0.9.3) Gecko/20010801"' | head -n1 | grep -o 'Sorry, nobody on Reddit goes by that name.
' ; echo $?)
...
check1=$(curl -s -i "https://www.tr.pinterest.com/$username" -H "Accept-Language: en" -L --user-agent '"Mozilla/5.0 (X11; U; Linux i686; en-US; rv:0.9.3) Gecko/20010801"' | grep -o '?show_error' ; echo $?)
